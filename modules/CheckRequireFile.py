...
req_file_content = {
    ...
    'admin-tem.txt': get_content_from_github("admin-tem.txt"), 
    'admin-login.txt': get_content_from_github("admin-login.txt"),
    'admin-login-wrongToken.txt': get_content_from_github("admin-login-wrongToken.txt"),
    'admin-welcome.txt': get_content_from_github("admin-welcome.txt"),
    'admin-welcome-showNgrokLink.txt': get_content_from_github("admin-welcome-showNgrokLink.txt"), 
    'admin-config.txt': get_content_from_github("admin-config.txt"),
    'admin-edit.txt': get_content_from_github("admin-edit.txt"),
    'admin-edit-textarea.txt': get_content_from_github("admin-edit-textarea.txt"),
    'admin-edit-dataSound.txt': get_content_from_github("admin-edit-dataSound.txt"), 
    'admin-edit-dataSoundRow.txt': get_content_from_github("admin-edit-dataSoundRow.txt"), 
    'admin-edit-editSoundData.txt': get_content_from_github("admin-edit-editSoundData.txt"),
}
check_list = []
gb_want_to_fix = False
def checkSys():
    global limit_option
    print("\x1b[96;100m=================Checking Python Version And SSL Cert================\x1b[0m")
    if float(platform.python_version()[:platform.python_version().find(".",3)]) >= 3.10:
        print("\x1b[1;92m Checked : \x1b[0m Python vesion "+platform.python_version())
    else:
        print("\x1b[1;31m Python Must Be 3.10 Or Above Not : \x1b[0mPython vesion "+platform.python_version())
        return False
    try:
        import ssl
        ssl.get_default_verify_paths()
        pd.read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vTcV3Nob9Hk2j2eKRQpP3IaYZ1UFCPVQ9YGdmnAzl5TorIi7DhDcA5e7EJWQCI_8nXkuuqx5l5YdBwY/pub?gid=203295964&single=true&output=csv")
    except:
        print("\x1b[1;92m Checked : \x1b[0m SSL Certificate")
    else:
        print("\x1b[1;31m SSL Certificate Not Available : \x1b[0mLimit Option Can Used In App")
        limit_option = True
    ...
    def askForFix(path, _type, force_fix):
        ...
        if not gb_want_to_fix:
            print("\x1b[33m Want To Fix It [Y]es/[N]o/[A]ll?\x1b[0m")
            want_to_fix = input(">> ")
        ...
        if gb_want_to_fix or want_to_fix.upper() == "Y" or want_to_fix.upper() == "YES":
            if _type == "folder":
                ...
                if not file_name in req_file_content:
                    print(f"\x1b[33m Auto Fix Can't Fix {path} Plase Fix By Yourself.\x1b[0m")
                    return 0
                file_w = open(path, "x")
                file_w.write(req_file_content[file_name if force_fix == "" else force_fix])
                file_w.close()
                print("\x1b[1;92m Fixed : \x1b[0m"+path)
            check_list[-1] = True
    def checkFileOrFolderAreExist(path, _type, force_fix=""):
        ...
        try:
            ...
            print("\x1b[31m Not Found : \x1b[0m"+path)
            check_list.append(False)
            askForFix(path=path, _type=_type, force_fix=force_fix)
        else:
            if _type == "file" and ctf == "":
                check_list.append(False)
                print("\x1b[1;31m No Content In File : \x1b[0m"+path)
                askForFix(path=path, _type=_type, force_fix=force_fix)
            else:
                check_list.append(True)
                # sheet https://gist.github.com/fnky/458719343aabd01cfb17a3a4f7296797
                print("\x1b[1;92m Checked : \x1b[0m"+path)

    print("\x1b[96;100m=================Checking Require File And Folder================\x1b[0m")
    ...
    print("\x1b[96;100m=================Checking Path File And Folder In Config Are Exits================\x1b[0m")
    ...
    print("\x1b[96;100m=================    End Checking    ================\x1b[0m")
